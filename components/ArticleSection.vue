<template>
  <div class="text-black dark:text-white" :class="editMode?`border border-gray-200 dark:border-gray-700 rounded-lg p-4 mb-4 mx-6bg-white dark:bg-gray-800`:(section.type !== 'image'?'py-1':'')">
    ...
    <div v-if="section.type === 'text'">
      ...
      <div v-else class=";max-w-none mx-6 md:mx-12 lg:mx-20 text-justify">
        <!-- <p v-for="(paragraph, index) in section.content.split('\n')" :key="index">{{ paragraph }}</p> -->
        <div class="" v-html="section.content.replaceAll(`<strong>`,`<b>`).replaceAll(`</strong>`,`</b>`).replaceAll(/<p><\/p>/g, '<p>&nbsp;</p>')"></div>
      </div>
    </div>
    ...
    <div v-if="section.type === 'subheading'">
      ...
      <div v-else class="prose dark:prose-invert max-w-none text-2xl font-semibold mx-6 md:mx-12 lg:mx-20" v-html="section.content"></div>
    </div>
