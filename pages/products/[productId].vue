<template>
    ...
    <div class="flex flex-row justify-center mt-6">
        <div class="flex flex-col py-5 px-8 bg-white dark:bg-gray-800 rounded-lg shadow-md mx-8">
            ...
            <div class="w-full flex flex-col items-center">
                ...
                <div class="grid grid-cols-2 md:grid-cols-3 gap-6 md:gap-12 lg:gap-16 xl:gap-20 justify-center py-5">
                    <div v-for="image in productData?.imageList" :key="image" class="cursor-pointer"
                        @click="openModal(image)">
                        <NuxtImg placeholder format="webp" :src="image"
                            class="w-full h-auto rounded-md shadow-md hover:scale-105 transition duration-200" />
                    </div>
                    <!-- youtube video -->
                    <iframe class="col-span-2 md:col-span-3 aspect-video min-w-full" :src="productData?.video">
                    </iframe>
                </div>

                <!-- enlarged image -->
                <button v-if="selectedImage" @click="selectedImage = null"
                    class="fixed inset-0 bg-black bg-opacity-80 flex items-center justify-center z-50">
                    <div class="relative bg-white p-2 rounded-lg">
                        <NuxtImg placeholder format="webp" :src="selectedImage"
                            class="max-w-[80vw] h-[80vh] object-contain" />
                    </div>
                </button>
            </div>
            ...
            <div class="flex flex-row">
                ...
                <a href="https://wa.me/6281388488430" class="font-semibold text-green-500 hover:underline"
